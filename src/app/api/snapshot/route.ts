...
/** Prisma singleton: prevents exhausting connections on serverless */
const prisma =
...
export async function GET(req: NextRequest) {
  ...
  if (!OPENWEATHER_API_KEY || !NPS_API_KEY) {
    return NextResponse.json(
      { error: 'Missing OPENWEATHER_API_KEY or NPS_API_KEY in environment' },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    return NextResponse.json({ savedId: saved.id, weather, activities });
  } catch (e) {
